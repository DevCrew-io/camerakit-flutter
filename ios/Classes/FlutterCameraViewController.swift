...
open class FlutterCameraViewController: UIViewController, CameraControllerUIDelegate {
    ...
    deinit {
        debugPrint("Deinit CameraViewController")
    }
    ...
    open func cameraController(_ controller: CameraController, updatedLenses lenses: [Lens]) {
        ...
        guard let selectedItem = cameraView?.carouselView.selectedItem else { return }
...
extension FlutterCameraViewController {
    private func setupCloseButton() {
        closeButton.addTarget(self, action: #selector(btnClose_Action), for: .touchUpInside)
    ...
    @objc private func btnClose_Action() {
        closeSnapchatSDK()
    }
    
    private func closeSnapchatSDK(dismiss: Bool = true) {
        clearLens()
    ...
    private func dismissVC() {
        ...
        // Deactivate the audio session to release mic
...
private extension FlutterCameraViewController {
    ...
    func setup() {
        ...
        setupActions()
        if let previewView = cameraView?.previewView {
...
extension FlutterCameraViewController {
    ...
    private func closeButtonPressed(_ sender: UIButton) {
        closeSnapchatSDK(dismiss: !lensId.isEmpty)
    }
...
extension FlutterCameraViewController {
    ...
    private func handleSingleTap(sender: UITapGestureRecognizer) {
        if cameraView?.isAnyControlVisible ?? false {
...
extension FlutterCameraViewController: AdjustmentsProcessorObserver {
    ...
    func updateAdjustmentButtonStatus() {
        cameraView?.cameraActionsView.toneMapActionView.isHidden = !(cameraController?.isToneMapAdjustmentAvailable ?? false)
        cameraView?.cameraActionsView.portraitActionView.isHidden = !(cameraController?.isPortraitAdjustmentAvailable ?? false)
    }
}
